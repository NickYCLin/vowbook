...
export function balancePrintData(items:readonly Balance[]):OperationsPrintData {
 ...
 return {
  summary:`${pending.length} 筆尾款待付 · 已登記尾款合計 ${formatTwdAmount(total)}${missing?` · ${missing} 筆金額待確認`:""}`,
  columns:[{label:"項目／廠商",width:25},{label:"聯絡資訊",width:20},{label:"尾款金額",width:14},{label:"付款期限",width:12},{label:"付款勾選",width:10},{label:"備註",width:19}],
  rows:pending.map(i=>({key:i.id,cells:[`${i.name}\n${i.confirmedVendor||"廠商待確認"}`,i.vendorContact??"",i.balanceAmount===null?"待確認":formatTwdAmount(i.balanceAmount),i.dueDate??"未設定",{checkLabel:"尾款付款勾選框"},[i.additionalAmount?`另登記追加費用 ${formatTwdAmount(i.additionalAmount)}（請核對是否已付）`:"",i.notes??""].filter(Boolean).join("\n")]})),
 };
}
