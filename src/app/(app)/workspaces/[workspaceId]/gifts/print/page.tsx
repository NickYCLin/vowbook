...
import {getWeddingGiftPrint} from "@/lib/wedding-cakes";
...
export default async function GiftPrintPage({params}:{params:Promise<{workspaceId:string}>}) {
 ...
 let data;try{data=await getWeddingGiftPrint(workspaceId);}catch(error){if(error instanceof WorkspaceAccessDeniedError)notFound();throw error;}
 return <main className="mx-auto w-full min-w-0 max-w-6xl px-5 py-6 sm:px-8 sm:py-12 print:m-0 print:max-w-none print:p-0">
  <div className="print:hidden">
   ...
   <p className="my-4 text-sm leading-6 text-ink-soft">沿用發餅名單的同一家人設定，每戶一列。包含所有受邀親友，不受出席回覆影響；新人本人、雙方父母與兄弟姊妹不列入（依已設定的關係稱謂辨識）。未登記者金額欄留白供現場手寫；已登記的家庭標示「禮金已收訖」，不顯示金額。已標記「不收禮金、會送餅」的親友不列入本表，仍依出席狀態列入發餅名單。</p>
